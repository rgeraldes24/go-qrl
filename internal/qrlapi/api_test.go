...
func testTransactionMarshal(t *testing.T, tests []txData, config *params.ChainConfig) {
	...
	for i, tt := range tests {
		...
		if data, err := json.Marshal(rpcTx); err != nil {
			...
			assertRPCTransactionJSON(t, i, data, tx, rpcTx)
		}
	}
}
...
func TestTransaction_RoundTripRpcJSON(t *testing.T) {
	var (
		...
		tests  = allTransactionTypes(common.BytesToAddress(common.Hex2Bytes("dead")), config)
	)
	testTransactionMarshal(t, tests, config)
}

func assertRPCTransactionJSON(t *testing.T, test int, data []byte, tx *types.Transaction, rpcTx *RPCTransaction) {
	t.Helper()

	var have map[string]any
	require.NoErrorf(t, json.Unmarshal(data, &have), "test %d: rpc json decode", test)

	require.Nilf(t, have["blockHash"], "test %d: blockHash", test)
	require.Nilf(t, have["blockNumber"], "test %d: blockNumber", test)
	require.Nilf(t, have["transactionIndex"], "test %d: transactionIndex", test)
	require.Equalf(t, rpcTx.From.Hex(), have["from"], "test %d: from", test)
	require.Lenf(t, have["from"], 1+common.AddressLength*2, "test %d: from length", test)
	if to := tx.To(); to == nil {
		require.Nilf(t, have["to"], "test %d: to", test)
	} else {
		require.Equalf(t, to.Hex(), have["to"], "test %d: to", test)
		require.Lenf(t, have["to"], 1+common.AddressLength*2, "test %d: to length", test)
	}

	require.Equalf(t, hexutil.EncodeUint64(tx.Gas()), have["gas"], "test %d: gas", test)
	require.Equalf(t, hexutil.EncodeBig((*big.Int)(rpcTx.GasPrice)), have["gasPrice"], "test %d: gasPrice", test)
	require.Equalf(t, tx.Hash().Hex(), have["hash"], "test %d: hash", test)
	require.Equalf(t, hexutil.Encode(tx.Data()), have["input"], "test %d: input", test)
	require.Equalf(t, hexutil.EncodeUint64(tx.Nonce()), have["nonce"], "test %d: nonce", test)
	require.Equalf(t, hexutil.EncodeBig(tx.Value()), have["value"], "test %d: value", test)
	require.Equalf(t, hexutil.EncodeUint64(uint64(tx.Type())), have["type"], "test %d: type", test)
	require.Equalf(t, hexutil.Encode(tx.Descriptor()), have["descriptor"], "test %d: descriptor", test)
	require.Equalf(t, hexutil.Encode(tx.ExtraParams()), have["extraParams"], "test %d: extraParams", test)
	require.Equalf(t, hexutil.Encode(tx.RawPublicKeyValue()), have["publicKey"], "test %d: publicKey", test)
	require.Equalf(t, hexutil.Encode(tx.RawSignatureValue()), have["signature"], "test %d: signature", test)

	if tx.Type() == types.DynamicFeeTxType {
		require.Equalf(t, hexutil.EncodeBig(tx.ChainId()), have["chainId"], "test %d: chainId", test)
		require.Equalf(t, hexutil.EncodeBig(tx.GasFeeCap()), have["maxFeePerGas"], "test %d: maxFeePerGas", test)
		require.Equalf(t, hexutil.EncodeBig(tx.GasTipCap()), have["maxPriorityFeePerGas"], "test %d: maxPriorityFeePerGas", test)
		requireAccessListJSON(t, test, have["accessList"], tx.AccessList())
	}
}

func requireAccessListJSON(t *testing.T, test int, have any, want types.AccessList) {
	t.Helper()

	accesses, ok := have.([]any)
	require.Truef(t, ok, "test %d: accessList type %T", test, have)
	require.Lenf(t, accesses, len(want), "test %d: accessList length", test)
	for i, access := range accesses {
		item, ok := access.(map[string]any)
		require.Truef(t, ok, "test %d: accessList[%d] type %T", test, i, access)
		require.Equalf(t, want[i].Address.Hex(), item["address"], "test %d: accessList[%d].address", test, i)
		require.Lenf(t, item["address"], 1+common.AddressLength*2, "test %d: accessList[%d].address length", test, i)

		keys, ok := item["storageKeys"].([]any)
		require.Truef(t, ok, "test %d: accessList[%d].storageKeys type %T", test, i, item["storageKeys"])
		require.Lenf(t, keys, len(want[i].StorageKeys), "test %d: accessList[%d].storageKeys length", test, i)
		for j, key := range keys {
			require.Equalf(t, want[i].StorageKeys[j].Hex(), key, "test %d: accessList[%d].storageKeys[%d]", test, i, j)
		}
	}
}
...
func TestRPCTransactionPreservesExtraParams(t *testing.T) {
	...
	var (
		config  = params.AllBeaconProtocolChanges
		signer  = types.LatestSigner(config)
		to      = common.BytesToAddress([]byte{0xaa, 0})
	...
	return []txData{
		{
			Tx: &types.DynamicFeeTx{
				...
				AccessList: types.AccessList{
					{
						Address:     common.BytesToAddress([]byte{0x2, 0}),
...
func TestRPCMarshalBlock(t *testing.T) {
	...
	for i, tc := range testSuite {
		...
		require.NoErrorf(t, err, "test %d: marshal block", i)

		var have map[string]any
		require.NoErrorf(t, json.Unmarshal(out, &have), "test %d: decode block", i)
		require.Equalf(t, block.Hash().Hex(), have["hash"], "test %d: hash", i)
		require.Equalf(t, block.Header().Coinbase.Hex(), have["miner"], "test %d: miner", i)
		require.Lenf(t, have["miner"], 1+common.AddressLength*2, "test %d: miner length", i)
		require.Equalf(t, hexutil.EncodeUint64(block.NumberU64()), have["number"], "test %d: number", i)
		require.Equalf(t, hexutil.EncodeUint64(uint64(block.Size())), have["size"], "test %d: size", i)
		require.Equalf(t, block.TxHash().Hex(), have["transactionsRoot"], "test %d: transactionsRoot", i)
		require.Equalf(t, block.ReceiptHash().Hex(), have["receiptsRoot"], "test %d: receiptsRoot", i)

		transactions, ok := have["transactions"].([]any)
		if !tc.inclTx {
			require.Falsef(t, ok, "test %d: unexpected transactions field", i)
			continue
		}
		require.Truef(t, ok, "test %d: transactions type %T", i, have["transactions"])
		require.Lenf(t, transactions, len(txs), "test %d: transactions length", i)
		for j, tx := range txs {
			if !tc.fullTx {
				require.Equalf(t, tx.Hash().Hex(), transactions[j], "test %d: transactions[%d]", i, j)
				continue
			}
			txFields, ok := transactions[j].(map[string]any)
			require.Truef(t, ok, "test %d: transactions[%d] type %T", i, j, transactions[j])
			require.Equalf(t, tx.Hash().Hex(), txFields["hash"], "test %d: transactions[%d].hash", i, j)
			require.Equalf(t, block.Hash().Hex(), txFields["blockHash"], "test %d: transactions[%d].blockHash", i, j)
			require.Equalf(t, hexutil.EncodeUint64(block.NumberU64()), txFields["blockNumber"], "test %d: transactions[%d].blockNumber", i, j)
			require.Equalf(t, hexutil.EncodeUint64(uint64(j)), txFields["transactionIndex"], "test %d: transactions[%d].transactionIndex", i, j)
			require.Equalf(t, common.Address{}.Hex(), txFields["from"], "test %d: transactions[%d].from", i, j)
			require.Lenf(t, txFields["from"], 1+common.AddressLength*2, "test %d: transactions[%d].from length", i, j)
			require.Equalf(t, to.Hex(), txFields["to"], "test %d: transactions[%d].to", i, j)
			require.Lenf(t, txFields["to"], 1+common.AddressLength*2, "test %d: transactions[%d].to length", i, j)
		}
	}
}
...
func TestRPCGetBlockOrHeader(t *testing.T) {
	...
	var (
		...
		withdrawal = &types.Withdrawal{
			Index:     0,
			Validator: 1,
			Address:   common.BytesToAddress(common.Hex2Bytes("1234")),
			Amount:    10,
		}
...
func setupReceiptBackend(t *testing.T, genBlocks int) (*testBackend, []common.Hash) {
	...
	var (
		...
		acc1Addr                     = acc1Wallet.GetAddress()
		acc2Addr      common.Address = acc2Wallet.GetAddress()
		contract                     = common.BytesToAddress(common.Hex2Bytes("31ec7"))
		genesis                      = &core.Genesis{
			Config: &config,
			Alloc: core.GenesisAlloc{
				acc1Addr: {Balance: big.NewInt(params.Quanta)},
...
func testRPCResponseWithFile(t *testing.T, testid int, result any, rpc string, file string) {
	...
	require.NoErrorf(t, json.Unmarshal(want, &wantJSON), "test %d: decode expected json", testid)
	require.NoErrorf(t, json.Unmarshal(data, &haveJSON), "test %d: decode actual json", testid)
	requireRPCAddressStrings(t, testid, haveJSON)
	normalizeRPCVolatileFields(wantJSON)
	normalizeRPCVolatileFields(haveJSON)
	require.Equalf(t, wantJSON, haveJSON, "test %d: json not match, want: %s, have: %s", testid, string(want), string(data))
}

func requireRPCAddressStrings(t *testing.T, testid int, value any) {
	t.Helper()

	switch value := value.(type) {
	case map[string]any:
		for _, child := range value {
			requireRPCAddressStrings(t, testid, child)
		}
	case []any:
		for _, child := range value {
			requireRPCAddressStrings(t, testid, child)
	...
	case string:
		if strings.HasPrefix(value, "Q") {
			require.Truef(t, common.IsAddress(value), "test %d: invalid 64-byte QRL address %q", testid, value)
		}
	}
}

func normalizeRPCVolatileFields(value any) {
	switch value := value.(type) {
	case map[string]any:
		for key, child := range value {
			switch key {
			case "hash", "blockHash", "parentHash", "transactionHash", "transactionsRoot", "signature":
				if child != nil {
					value[key] = "<volatile-hash>"
				}
			case "transactions":
				normalizeRPCTransactions(child)
			default:
				normalizeRPCVolatileFields(child)
			}
		}
	case []any:
		for _, child := range value {
...
func normalizeRPCTransactions(value any) {
	transactions, ok := value.([]any)
	if !ok {
		return
	}
	for i, tx := range transactions {
		if s, ok := tx.(string); ok && isRPCHashString(s) {
			transactions[i] = "<volatile-hash>"
			continue
		}
		normalizeRPCVolatileFields(tx)
	}
}

func isRPCHashString(s string) bool {
