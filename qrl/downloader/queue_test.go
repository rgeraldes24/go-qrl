...
func makeChain(n int, seed byte, parent *types.Block, empty bool) ([]*types.Block, []types.Receipts) {
	blocks, receipts := core.GenerateChain(params.TestChainConfig, parent, beacon.NewFaker(), testDB, n, func(i int, block *core.BlockGen) {
		...
		if !empty && i%2 == 0 {
			...
			tx, err := types.SignTx(types.NewTx(&types.MLDSA87Tx{Nonce: block.TxNonce(testAddress), To: &common.Address{seed}, Value: big.NewInt(1000), Gas: params.TxGas, GasFeeCap: big.NewInt(875000000), Data: nil}), signer, testKey)
			if err != nil {
				panic(err)
			}
			block.AddTx(tx)
