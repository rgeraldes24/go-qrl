...
// pledgeAddressHex (below, 48 hex bytes left-padded in the 64-byte ABI slot), 2218516807680, "usd"
...
func TestEventSignatureTopic(t *testing.T) {
	t.Parallel()

	event := NewEvent("Called", "Called", false, nil)
	want := common.HashToLogTopic(crypto.Keccak256Hash([]byte("Called()")))
	if event.SignatureTopic() != want {
		t.Fatalf("event signature topic mismatch: got %x want %x", event.SignatureTopic(), want)
	}
}
