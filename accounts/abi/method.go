...
type Method struct {
	...
	// Constant and Payable are legacy indicators emitted by compilers before
	// stateMutability was introduced. Keep them so older ABI metadata retains
	// its meaning.
	Constant bool
	Payable  bool
...
func NewMethod(name string, rawName string, funType FunctionType, mutability string, inputs Arguments, outputs Arguments) Method {
	return newMethod(name, rawName, funType, mutability, false, false, inputs, outputs)
}

// newMethod constructs a method while retaining legacy compiler metadata.
// NewMethod keeps its existing public signature; JSON decoding uses this helper
// when the deprecated constant/payable fields are present.
