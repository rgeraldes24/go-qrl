...
func TestUnpackIndexedDynamicTyLogIntoStruct(t *testing.T) {
	...
	topics := []common.LogTopic{
		common.HashToLogTopic(crypto.Keccak256Hash([]byte("received(string,bytes,address,uint256,bytes)"))),
		common.HashToLogTopic(nameHash),
		common.HashToLogTopic(contentHash),
	}
	mockLog := newMockLog(topics, common.HexToHash("0x0"))
	...
	if want := common.HashToLogTopic(nameHash); received.Name != want {
		t.Fatalf("name topic mismatch: have %v, want %v", received.Name, want)
	}
	if want := common.HashToLogTopic(contentHash); received.Content != want {
