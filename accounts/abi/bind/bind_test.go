...
// The large Solidity bytecode blobs that originally lived in this table were
// Ethereum-era compiler output. They contained 32-byte ABI/panic payload
...
var bindTests = []struct {
	...
	// Test that the binding is available in combined and separate forms too
	{
		...
		[]string{compileOnlyVM64Bytecode},
		[]string{`[]`},
		`
			"github.com/theQRL/go-qrl/common"
		`,
	...
	// Tests that named, anonymous and indexed events are handled correctly
	{
		...
		`
			if e, err := NewEventChecker(common.Address{}, nil); e == nil || err != nil {
				...
				dit, err := e.FilterDynamic(nil, []common.LogTopic{}, []common.LogTopic{})
				...
				topic = dit.Event.IdxStr // Make sure indexed strings expose full-width topics
				topic = dit.Event.IdxDat // Make sure indexed bytes expose full-width topics
	...
	// Tests that logs can be successfully filtered and decoded.
	{
		...
		`
			...
			helloTopic := common.HexToLogTopic("0x06b3dfaec148fb1bb2b066f10ec285e7c9bf402ab32aa78a5d38e34566810cd2")
			worldTopic := common.HexToLogTopic("0xf2208c967df089f60420785795c0a9ba8896b0f6f1867fa7f1f12ad6f79c1a18")
			dit, err := eventer.FilterDynamicEvent(nil, []common.LogTopic{helloTopic}, []common.LogTopic{worldTopic})
			if err != nil {
			...
			if dit.Event.NonIndexedString != "Hello" || string(dit.Event.NonIndexedBytes) != "World" || dit.Event.IndexedString != helloTopic || dit.Event.IndexedBytes != worldTopic {
	...
	},
	{
		...
		`,
		bytecode: []string{compileOnlyVM64Bytecode},
...
func TestBindRejectsUnsupportedFunctionType(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "TupleInput",
			abi:  `[{"inputs":[{"components":[{"internalType":"function (uint256) external","name":"callback","type":"function"}],"internalType":"struct CallbackConfig","name":"config","type":"tuple"}],"name":"test","outputs":[],"stateMutability":"nonpayable","type":"function"}]`,
		},
		{
			name: "ArrayInput",
		...
		{
			name: "ErrorInput",
			abi:  `[{"inputs":[{"internalType":"function (uint256) external","name":"callback","type":"function"}],"name":"BadCallback","type":"error"}]`,
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			_, err := Bind([]string{tt.name}, []string{tt.abi}, []string{""}, nil, "bindtest", nil, nil)
			if !errors.Is(err, qrlabi.ErrUnsupportedFunctionType) {
				t.Fatalf("Bind error = %v, want %v", err, qrlabi.ErrUnsupportedFunctionType)
			}
		})
	}
}

func TestBindVM64LibraryLinking(t *testing.T) {
	t.Parallel()

	pattern := LibraryLinkPattern("Math.sol:Math")
	placeholder := libraryPlaceholder(pattern)
	if len(pattern) != libraryPlaceholderPatternHexLength {
		t.Fatalf("VM64 library pattern length = %d, want %d", len(pattern), libraryPlaceholderPatternHexLength)
	}
	if len(placeholder) != common.AddressLength*2 {
		t.Fatalf("VM64 library placeholder length = %d, want %d", len(placeholder), common.AddressLength*2)
	}

	code, err := Bind(
		[]string{"UseMath", "Math"},
		[]string{`[]`, `[]`},
		[]string{"9f" + placeholder + "00", "00"},
		nil,
		"bindtest",
	...
	want := fmt.Sprintf(`UseMathBin = strings.ReplaceAll(UseMathBin, "%s", mathAddr.String()[1:])`, placeholder)
	if !strings.Contains(code, want) {
		t.Fatalf("generated binding does not link VM64 placeholder:\nwant fragment: %s\ncode:\n%s", want, code)
	}
}

func TestBindRejectsUnresolvedLibraryPlaceholders(t *testing.T) {
	t.Parallel()

	pattern := LibraryLinkPattern("Math.sol:Math")
	_, err := Bind([]string{"UseMath"}, []string{`[]`}, []string{"9f" + libraryPlaceholder(pattern)}, nil, "bindtest", nil, nil)
	if !errors.Is(err, ErrUnresolvedLibraryLink) {
		t.Fatalf("Bind error = %v, want %v", err, ErrUnresolvedLibraryLink)
	}
}
...
func TestBindBytes64FixedBytesType(t *testing.T) {
	t.Parallel()

	code, err := Bind(
		[]string{"Bytes64"},
		[]string{`[{"inputs":[{"internalType":"bytes64","name":"x","type":"bytes64"}],"name":"roundTrip","outputs":[{"internalType":"bytes64","name":"","type":"bytes64"}],"stateMutability":"view","type":"function"}]`},
		[]string{""},
		nil,
		"bindtest",
		nil,
	...
	if !strings.Contains(code, "[64]byte") {
		t.Fatalf("generated binding does not use [64]byte for bytes64:\n%s", code)
	}
}

func TestBindIndexedDynamicEventTopicsUseLogTopic(t *testing.T) {
	t.Parallel()

	code, err := Bind(
		[]string{"Topics"},
		[]string{`[{"anonymous":false,"inputs":[{"indexed":true,"name":"indexedString","type":"string"},{"indexed":true,"name":"indexedBytes","type":"bytes"},{"indexed":true,"name":"indexedNames","type":"string[]"},{"indexed":true,"name":"indexedAddresses","type":"address[2]"},{"components":[{"name":"x","type":"uint256"},{"name":"y","type":"uint256"}],"indexed":true,"internalType":"struct Topics.Point","name":"indexedTuple","type":"tuple"},{"indexed":true,"name":"indexedFixedBytes","type":"bytes24"}],"name":"Observed","type":"event"}]`},
		[]string{""},
		nil,
		"bindtest",
		nil,
	...
	for _, want := range []struct {
		...
		{"IndexedString", "common.LogTopic"},
		{"IndexedBytes", "common.LogTopic"},
		{"IndexedNames", "common.LogTopic"},
		{"IndexedAddresses", "common.LogTopic"},
		{"IndexedTuple", "common.LogTopic"},
		{"IndexedFixedBytes", "[24]byte"},
	} {
		if !generatedFieldExists(code, want.name, want.typ) {
			t.Fatalf("generated binding missing field %s %s:\n%s", want.name, want.typ, code)
		}
	}
	for _, want := range []string{
		...
		if !strings.Contains(code, want) {
			t.Fatalf("generated binding missing indexed filter parameter %q:\n%s", want, code)
		}
	}
	for _, stale := range []string{
		...
		"indexedBytes [][]byte",
		"indexedNames [][]string",
		"indexedAddresses [][2]common.Address",
	} {
		if strings.Contains(code, stale) {
			t.Fatalf("generated binding still contains stale indexed filter parameter %q:\n%s", stale, code)
		}
	}
}

func generatedFieldExists(code string, name string, typ string) bool {
	for _, line := range strings.Split(code, "\n") {
		fields := strings.Fields(line)
		if len(fields) >= 2 && fields[0] == name && fields[1] == typ {
			return true
		}
	}
	return false
...
func usesCompileOnlyVM64Bytecode(bytecodes []string) bool {
	for _, bytecode := range bytecodes {
		bin := strings.TrimPrefix(strings.TrimSpace(bytecode), "0x")
		if bin == compileOnlyVM64Bytecode {
			return true
		}
	}
	return false
...
func compileOnlyRuntimeTestBody(tester string) string {
	return fmt.Sprintf("t.Skip(%q)\n%s", compileOnlyVM64RuntimeSkip, tester)
}

func TestBindFixturesUseOnlyCompileOnlyBytecode(t *testing.T) {
	t.Parallel()

	for _, tt := range bindTests {
		for i, bytecode := range tt.bytecode {
			bin := strings.TrimPrefix(strings.TrimSpace(bytecode), "0x")
			if bin == "" || bin == compileOnlyVM64Bytecode {
				continue
			}
...
// type-checked. Concrete Solidity bytecode fixtures must be regenerated from
// Hyperion compiler output before this test can safely execute runtime
// assertions.
func TestGolangBindings(t *testing.T) {
	...
	for i, tt := range bindTests {
		t.Run(tt.name, func(t *testing.T) {
			...
			tester := tt.tester
			if usesCompileOnlyVM64Bytecode(tt.bytecode) {
				tester = compileOnlyRuntimeTestBody(tester)
			}
			code := fmt.Sprintf(`
				package bindtest
	...
	// Verify the generated bindings compile cleanly (go vet performs a full
	// type-check + compilation without running anything). We deliberately skip
	// `go test` here: the fixture bytecode is compile-only placeholder bytecode
	// until the Solidity snippets are regenerated with Hyperion VM64 codegen.
	// The value of this test is proving the binder still emits valid Go against
	// the current types, not pretending old Solidity output is executable VM64
	// bytecode.
