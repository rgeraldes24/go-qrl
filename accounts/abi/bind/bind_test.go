...
// TestBindFunctionType checks that abigen binds Hyperion external function
// values as [68]byte ([common.AddressLength+4]byte: address || selector)
// wherever they appear, and as the verbatim hash topic when indexed.
func TestBindFunctionType(t *testing.T) {
	t.Parallel()
	...
	tests := []struct {
		...
		},
		{
			...
			abi:  `[{"anonymous":false,"inputs":[{"indexed":true,"internalType":"function (uint256) external","name":"callback","type":"function"}],"name":"Test","type":"event"}]`,
			want: "Callback common.LogTopic",
		},
		{
			name: "non-indexed event input",
			abi:  `[{"anonymous":false,"inputs":[{"indexed":false,"internalType":"function (uint256) external","name":"callback","type":"function"}],"name":"Test","type":"event"}]`,
			want: "Callback [68]byte",
		},
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// Non-empty bytecode so the constructor's Deploy method is emitted.
			code, err := Bind([]string{"FunctionType"}, []string{tt.abi}, []string{"0x00"}, nil, "bindtest", nil, nil)
			if err != nil {
				t.Fatalf("Bind error = %v", err)
			}
			if !strings.Contains(code, tt.want) {
				t.Fatalf("generated binding does not contain %q", tt.want)
			}
		})
	}
}
