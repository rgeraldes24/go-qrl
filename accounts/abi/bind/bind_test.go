...
func TestBindTagsRenamedEventFields(t *testing.T) {
	code, err := Bind(
		[]string{"EventFields"},
		[]string{`[{"anonymous":false,"inputs":[{"indexed":false,"name":"msg","type":"uint256"},{"indexed":true,"name":"_msg","type":"uint256"},{"indexed":true,"name":"range","type":"uint256"}],"name":"Renamed","type":"event"}]`},
		[]string{"00"},
		nil,
		"bindtest",
		nil,
	...
	for _, want := range []string{
		"(?m)^\\s*Msg\\s+\\*big.Int\\s+`abi:\"msg\"`$",
		"(?m)^\\s*Msg0\\s+\\*big.Int\\s+`abi:\"_msg\"`$",
		"(?m)^\\s*Arg2\\s+\\*big.Int\\s+`abi:\"range\"`$",
	} {
