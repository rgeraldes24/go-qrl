...
// covered by the Hyperion-compiled VM64 binding in scripts/local_testnet/goabi.
func TestGolangBindingsCompile(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("failed to resolve temporary workspace: %v", err)
	}
	...
	// Convert the generated package to a module and use this checkout directly.
	// Dependency-module replacements are not transitive, so copy the root
	// module's replacements into the temporary module instead of hard-coding the
	// current go-qrllib fork here.
	...
	pwd, err := os.Getwd()
	if err != nil {
		t.Fatalf("failed to resolve bind package directory: %v", err)
	}
	repoRoot := filepath.Clean(filepath.Join(pwd, "..", "..", ".."))
	rootModData, err := os.ReadFile(filepath.Join(repoRoot, "go.mod"))
	if err != nil {
		t.Fatalf("failed to read root go.mod: %v", err)
	}
	rootMod, err := modfile.Parse("go.mod", rootModData, nil)
	if err != nil {
		t.Fatalf("failed to parse root go.mod: %v", err)
	}
	editArgs := []string{
		"mod", "edit",
		"-require=github.com/theQRL/go-qrl@v0.0.0",
		"-replace=github.com/theQRL/go-qrl=" + repoRoot,
	}
	for _, replacement := range rootMod.Replace {
		oldPath := replacement.Old.Path
		if replacement.Old.Version != "" {
			oldPath += "@" + replacement.Old.Version
		}
		...
		editArgs = append(editArgs, "-replace="+oldPath+"="+newPath)
	}
	editor := exec.Command(gocmd, editArgs...)
	editor.Dir = pkg
	editor.Env = append(os.Environ(), "GOWORK=off")
	if out, err := editor.CombinedOutput(); err != nil {
		t.Fatalf("failed to configure generated binding module: %v\n%s", err, out)
	}
	...
	cmd := exec.Command(gocmd, "vet", "-mod=mod", "./...")
	cmd.Dir = pkg
