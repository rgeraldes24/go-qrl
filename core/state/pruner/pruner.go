...
func (p *Pruner) Prune(root common.Hash) error {
	...
	if !rawdb.HasLegacyTrieNode(p.db, root) {
		// Consecutive blocks can share a state root when a block does not
		// modify state. In that case a snapshot difflayer is not created, so
		// HEAD-127 may not be paired with the head-127 layer. The paired layer
		// is higher than the bottom-most diff layer. Try to find the
		// bottom-most snapshot layer with state available.
		//
