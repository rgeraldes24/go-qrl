...
// Memory implements a simple memory model for the quantum resistant virtual machine.
type Memory struct {
	store       []byte
	lastGasCost uint64
}
...
// Set64 sets one VM word starting at offset to the value of val.
func (m *Memory) Set64(offset uint64, val *uint512.Int) {
	// length of store may never be less than offset + size.
	// The store should be resized PRIOR to setting the memory
	if offset+uint512.WordBytes > uint64(len(m.store)) {
		panic("invalid memory: store empty")
	}
	// Fill in relevant bits
	b64 := val.Bytes64()
	copy(m.store[offset:], b64[:])
}
