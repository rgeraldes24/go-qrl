...
// This method may return false positives if Type is not a valid expression.
func (t *Type) isArray() bool {
	return strings.IndexByte(t.Type, '[') > 0
}

// typeName returns the canonical name of the type. If the type is 'Person[]' or
...
func (typedData *TypedData) EncodeData(primaryType string, data map[string]any, depth int) (hexutil.Bytes, error) {
	...
	for _, field := range typedData.Types[primaryType] {
		...
		if encType[len(encType)-1:] == "]" {
			...
			buffer.Write(encodeHashWord(encodedData))
...
func (typedData *TypedData) encodeArrayValue(encValue any, encType string, depth int) (hexutil.Bytes, error) {
	...
	for _, item := range arrayValue {
		if reflect.TypeOf(item).Kind() == reflect.Slice ||
			...
			if reflect.TypeOf(item).Elem().Kind() == reflect.Uint8 {
				...
				if err == nil {
					encodedData = encodeHashWord(encodedData)
				}
			}
			if err != nil {
				return nil, err
		...
		} else if typedData.Types[parsedType] != nil {
