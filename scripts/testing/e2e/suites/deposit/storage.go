...
func verifyDepositBranchStorage(ctx context.Context, client *qrlclient.Client, contract common.Address, leaves [][32]byte, block *big.Int) error {
	...
	for pair := 0; pair < depositBranchSlotCount; pair++ {
		...
		got, err := client.StorageAt(ctx, contract, key, block)
		if err != nil {
