...
On a fresh topology, the check verifies that:
...
The lifecycle harness runs it after the strict RPC suites and before any
restart/fresh-sync phase. For focused development against an already running
fresh network, the standalone command is:

```sh
go -C scripts/testing/e2e run ./cmd/depositcheck \
...
The standalone command has no checkpoint integration and is intentionally
one-shot. The lifecycle harness durably stores each signed raw transaction
before broadcast and records its hash before waiting for receipts or beacon
ingestion. Use `vm64e2e resume` with the existing checkpoint and enclave after
an interruption. Resume validates the exact sender, chain ID, nonce, deposit
contract, value, calldata, and hash; looks up or rebroadcasts only those exact
signed bytes; settles the recorded prefix; and continues from the first
unfinished deposit. It never recreates the network or repeats a completed
deposit.

An unjournaled pending transaction remains a fail-closed condition: inspect it
and restore the matching checkpoint rather than starting the suite again.
