...
func TestParseConfigRequiresSerializedPhaseAndPreservesDefaults(t *testing.T) {
	if _, err := ParseConfig(nil); err == nil || !strings.Contains(err.Error(), "-phase is required") {
		t.Fatalf("ParseConfig without -phase error = %v, want required serialized phase", err)
	}
	if _, err := ParseConfig([]string{"-phase", "all"}); err == nil || !strings.Contains(err.Error(), "not a serialized compatibility phase") {
		t.Fatalf("ParseConfig with -phase all error = %v, want serialized-phase error", err)
	}
	for _, phase := range []Phase{PhaseBase, PhaseSignerRestart, PhaseParticipantRestart} {
		cfg, err := ParseConfig([]string{"-phase", string(phase)})
		if err != nil {
			t.Fatalf("ParseConfig phase %q: %v", phase, err)
		}
		if cfg.Phase != phase {
			t.Fatalf("ParseConfig phase = %q, want %q", cfg.Phase, phase)
		}
	}
	...
	if cfg.phase != string(PhaseBase) {
		t.Fatalf("phase = %q, want base", cfg.phase)
	}
	...
	strict, err := parseConfig([]string{"-phase", string(PhaseBase), "-require-zero-duty-history"})
	if err != nil {
		t.Fatal(err)
	}
...
func TestParseConfigRejectsUnsafeTransfer(t *testing.T) {
	_, err := parseConfig([]string{"-phase", string(PhaseBase), "-recipient", defaultSignerAddress})
	if err == nil || !strings.Contains(err.Error(), "must differ") {
		t.Fatalf("got %v, want distinct-address error", err)
	}
	_, err = parseConfig([]string{"-phase", string(PhaseBase), "-value", "0"})
	if err == nil || !strings.Contains(err.Error(), "positive") {
		t.Fatalf("got %v, want positive-value error", err)
	}
	_, err = parseConfig([]string{"-phase", string(PhaseBase), "-validator-poll", "0s"})
	...
	_, err = parseConfig([]string{"-phase", string(PhaseBase), "-validator-poll", validatorDutyObservationTimeout.String()})
	...
	_, err = parseConfig([]string{"-phase", "unknown"})
	if err == nil || !strings.Contains(err.Error(), "invalid phase") {
		t.Fatalf("got %v, want invalid-phase error", err)
	}
	_, err = parseConfig([]string{"-phase", "participant-restart", "-skip-restarts"})
	if err == nil || !strings.Contains(err.Error(), "unavailable to serialized compatibility phases") {
		t.Fatalf("got %v, want phase/skip-restarts incompatibility", err)
	}
}

func TestRunSystemCheckAppliesWholeRunTimeout(t *testing.T) {
	cfg, err := parseConfig([]string{"-phase", string(PhaseBase), "-timeout", "20ms"})
	if err != nil {
		t.Fatal(err)
	}
...
func TestParseServiceStatusIsTriState(t *testing.T) {
	t.Parallel()
	for _, test := range []struct {
		input string
		want  ServiceStatus
	}{
		{input: `{"service_status":"RUNNING"}`, want: ServiceRunning},
		{input: `{"service":{"status":"STOPPED"}}`, want: ServiceStopped},
	} {
		got, err := parseServiceStatus(test.input)
		if err != nil || got != test.want {
			t.Fatalf("parseServiceStatus(%s) = %q, %v; want %q", test.input, got, err, test.want)
		}
	}
	for _, input := range []string{`{"status":"UNKNOWN"}`, `{"message":"temporary failure"}`, `not-json`} {
		if _, err := parseServiceStatus(input); err == nil {
			t.Fatalf("unknown service status %q was accepted", input)
		}
	}
}
...
func TestSignerReadinessRetriesDiscoveryAndCommitsHealthyEndpoint(t *testing.T) {
	...
	if len(recorder.endpoints) != 1 || recorder.endpoints[0].Service != cfg.signerSvc || recorder.endpoints[0].Kind != "signer-http" || recorder.endpoints[0].Previous != oldHTTP.URL || recorder.endpoints[0].Current != signerHTTP.URL {
		t.Fatalf("unexpected signer endpoint evidence: %+v", recorder.endpoints)
	}
}
...
func TestRestartSignerDoesNotDoubleStartAfterReadinessFailure(t *testing.T) {
	...
	check := &systemCheck{
		...
		observations: SystemObservationRecorderFunc(func(context.Context, string, string, time.Time) error {
			return nil
		}),
	...
	requireRestartStates(t, recorder.restarts, []RestartState{RestartStopIntent, RestartStopped, RestartStartIntent, RestartStarted})
}
...
type delayedSignerSideEffectAPI struct {
	nonce atomic.Uint64
}

func (*delayedSignerSideEffectAPI) BlockNumber() hexutil.Uint64 { return 30 }

func (api *delayedSignerSideEffectAPI) GetTransactionCount(context.Context, common.Address, rpc.BlockNumberOrHash) hexutil.Uint64 {
	return hexutil.Uint64(api.nonce.Load())
}

func (*delayedSignerSideEffectAPI) GetBalance(context.Context, common.Address, rpc.BlockNumberOrHash) *hexutil.Big {
	return (*hexutil.Big)(big.NewInt(10))
}

func TestDurableSignerQuiescenceIsRevalidatedBeforeRecoveryTransfer(t *testing.T) {
	api := new(delayedSignerSideEffectAPI)
	api.nonce.Store(5)
	server := rpc.NewServer()
	if err := server.RegisterName("qrl", api); err != nil {
		t.Fatal(err)
	...
	milestone, err := json.Marshal(assertionMilestone{Version: 1, Completed: true})
	if err != nil {
		t.Fatal(err)
	}
	check := &systemCheck{
		...
		clients: [2]*qrlclient.Client{client, client},
		resume: resumeState{observations: map[string]string{
			signerQuiescenceAssertedObservation: string(milestone),
		}},
	}
	baseline := [2]managedAccountState{
		{head: 10, nonce: 4, pendingNonce: 4, recipientBalance: big.NewInt(10)},
	...
	if err := check.assertCanceledSignerRequestQuiescent(t.Context(), baseline); err == nil || !strings.Contains(err.Error(), "delayed transaction side effect") {
		t.Fatalf("stale quiescence error = %v", err)
	}
}

type CaptureManagedTransferArgs struct {
	Value *hexutil.Big `json:"value"`
}

type captureManagedTransferAPI struct {
	values []*big.Int
}

func (api *captureManagedTransferAPI) SendTransaction(_ context.Context, args CaptureManagedTransferArgs) common.Hash {
	api.values = append(api.values, new(big.Int).Set((*big.Int)(args.Value)))
	return common.HexToHash(fmt.Sprintf("0x%x", len(api.values)))
}

func TestSignerOutageProbeCannotMasqueradeAsRecoveryTransfer(t *testing.T) {
	api := new(captureManagedTransferAPI)
	server := rpc.NewServer()
	if err := server.RegisterName("qrl", api); err != nil {
		t.Fatal(err)
	}
	...
	if _, err := check.sendManagedTransfer(t.Context(), 0, TransactionLabelSignerOutageProbe); err != nil {
		t.Fatal(err)
	}
	if _, err := check.sendManagedTransfer(t.Context(), 0, TransactionLabelSignerRecoveryTransfer); err != nil {
		t.Fatal(err)
	}
	if len(api.values) != 2 || api.values[0].Cmp(big.NewInt(8)) != 0 || api.values[1].Cmp(big.NewInt(7)) != 0 {
		t.Fatalf("outage/recovery transfer values = %v, want [8 7]", api.values)
	}
}
...
func TestParticipantReadinessCommitsRediscoveredEndpoints(t *testing.T) {
	...
	newCLMux.HandleFunc(peerCountPath, func(w http.ResponseWriter, _ *http.Request) {
		fmt.Fprint(w, `{"data":{"connected":"1"}}`)
	})
	newCLMux.HandleFunc(finalityPath, func(w http.ResponseWriter, _ *http.Request) {
...
func TestPreValidatorBeaconRecoveryRejectsFinal14StaleHead(t *testing.T) {
	base := [2]beaconStatus{
		{headSlot: 925, finalizedEpoch: 6, finalizedRoot: "0xabc", connectedPeers: 1},
		{headSlot: 927, finalizedEpoch: 6, finalizedRoot: "0xabc", connectedPeers: 1},
	}
	if err := validatePreValidatorBeaconRecovery(base, 915); err != nil {
		t.Fatalf("healthy recovery rejected: %v", err)
	}

	for _, test := range []struct {
		name string
		edit func(*[2]beaconStatus)
		want string
	}{
		{name: "final14 stale CL2", edit: func(got *[2]beaconStatus) { got[1].headSlot = 915 }, want: "CL2 head slot 915 has not advanced"},
		{name: "CL2 behind CL1", edit: func(got *[2]beaconStatus) { got[1].headSlot = 924 }, want: "behind CL1"},
		{name: "finality root mismatch", edit: func(got *[2]beaconStatus) { got[1].finalizedRoot = "0xdef" }, want: "finalized checkpoints differ"},
		{name: "finality epoch mismatch", edit: func(got *[2]beaconStatus) { got[1].finalizedEpoch++ }, want: "finalized checkpoints differ"},
	} {
		t.Run(test.name, func(t *testing.T) {
			got := base
			test.edit(&got)
			if err := validatePreValidatorBeaconRecovery(got, 915); err == nil || !strings.Contains(err.Error(), test.want) {
...
func TestBeaconGenesisAndProposerDuties(t *testing.T) {
	pubkey := "0x" + strings.Repeat("ab", 2592)
	mux := http.NewServeMux()
	mux.HandleFunc(genesisPath, func(w http.ResponseWriter, _ *http.Request) {
		fmt.Fprint(w, `{"data":{"genesis_time":"1700000000"}}`)
	})
	mux.HandleFunc(fmt.Sprintf(proposerDutiesPath, 2), func(w http.ResponseWriter, _ *http.Request) {
		fmt.Fprintf(w, `{"dependent_root":"0xAABB","execution_optimistic":false,"data":[{"pubkey":%q,"validator_index":"64","slot":"256"},{"pubkey":"0xCAFE","validator_index":"65","slot":"257"}]}`, strings.ToUpper(pubkey))
	})
	server := httptest.NewServer(mux)
	defer server.Close()
	reader := httpReader{client: server.Client()}
	genesis, err := reader.beaconGenesisTime(t.Context(), server.URL)
	if err != nil {
		t.Fatal(err)
	}
	if genesis != 1700000000 {
		t.Fatalf("genesis time = %d", genesis)
	}
	set, err := reader.proposerDuties(t.Context(), server.URL, 2)
	if err != nil {
		t.Fatal(err)
	}
	if set.dependentRoot != "0xaabb" || len(set.duties) != 2 {
		t.Fatalf("unexpected duty set: %+v", set)
	}
	if set.duties[0].pubkey != pubkey || set.duties[0].validatorIndex != 64 || set.duties[0].slot != 256 {
		t.Fatalf("full-width proposer duty was not preserved: %+v", set.duties[0])
	}
}

func TestProposerDutyAgreement(t *testing.T) {
	base := proposerDutySet{
		dependentRoot: "0xabc",
		duties: []proposerDuty{
			{pubkey: "0x01", validatorIndex: 64, slot: 128},
			{pubkey: "0x02", validatorIndex: 65, slot: 129},
		},
	}
	clone := func() proposerDutySet {
		return proposerDutySet{dependentRoot: base.dependentRoot, duties: append([]proposerDuty(nil), base.duties...)}
	}
	if err := equalProposerDutySets(base, clone()); err != nil {
		t.Fatal(err)
	}
	for _, test := range []struct {
		name string
		edit func(*proposerDutySet)
		want string
	}{
		{name: "dependent root", edit: func(got *proposerDutySet) { got.dependentRoot = "0xdef" }, want: "dependent roots differ"},
		{name: "count", edit: func(got *proposerDutySet) { got.duties = got.duties[:1] }, want: "counts differ"},
		{name: "mapping", edit: func(got *proposerDutySet) { got.duties[0].validatorIndex++ }, want: "duty 0 differs"},
	} {
		t.Run(test.name, func(t *testing.T) {
			got := clone()
			test.edit(&got)
			if err := equalProposerDutySets(base, got); err == nil || !strings.Contains(err.Error(), test.want) {
				t.Fatalf("got %v, want error containing %q", err, test.want)
			}
		})
	}
...
func TestProposerStartupSelectionAcrossEpochBoundary(t *testing.T) {
	currentSlot := uint64(127)
	duties := []proposerDuty{
		{pubkey: "vc1", validatorIndex: 1, slot: 128},
		{pubkey: "VC2", validatorIndex: 64, slot: 131},
		{pubkey: "vc2", validatorIndex: 65, slot: 140},
	}
	duty, err := nextOwnedProposerDuty(duties, map[string]struct{}{"vc2": {}}, currentSlot)
	if err != nil {
		t.Fatal(err)
	}
	if duty.slot != 131 || duty.validatorIndex != 64 {
		t.Fatalf("selected duty %+v", duty)
	}
	if lead := duty.slot - currentSlot; lead != validatorStartupLeadSlots {
		t.Fatalf("lead = %d, want %d", lead, validatorStartupLeadSlots)
	}
	if err := validateValidatorProposalLead(currentSlot, duty, validatorStartupLeadSlots); err != nil {
		t.Fatal(err)
	}
	tooClose := duty
	tooClose.slot = currentSlot + validatorStartupLeadSlots - 1
	if err := validateValidatorProposalLead(currentSlot, tooClose, validatorStartupLeadSlots); err == nil || !strings.Contains(err.Error(), "only 3 slots") {
		t.Fatalf("got %v, want insufficient-lead rejection", err)
	}
	if slot, err := beaconSlotAt(time.Unix(1700000000+127*int64(beaconSecondsPerSlot), 0), 1700000000); err != nil || slot != 127 {
		t.Fatalf("wall-clock slot = %d, err=%v", slot, err)
	}
}

func TestWaitValidatorProposalWindowResamplesAfterSlowStatusReads(t *testing.T) {
	pubkey := "0x" + strings.Repeat("ab", 2592)
	newBeacon := func(t *testing.T) *httptest.Server {
		t.Helper()
		...
		mux.HandleFunc(fmt.Sprintf(proposerDutiesPath, 0), func(w http.ResponseWriter, _ *http.Request) {
			fmt.Fprintf(w, `{"dependent_root":"0xabc","execution_optimistic":false,"data":[{"pubkey":%q,"validator_index":"64","slot":"104"},{"pubkey":%q,"validator_index":"64","slot":"107"}]}`, pubkey, pubkey)
		})
		mux.HandleFunc(fmt.Sprintf(proposerDutiesPath, 1), func(w http.ResponseWriter, _ *http.Request) {
			fmt.Fprint(w, `{"dependent_root":"0xdef","execution_optimistic":false,"data":[{"pubkey":"0x01","validator_index":"1","slot":"128"}]}`)
		})
		mux.HandleFunc(syncStatusPath, func(w http.ResponseWriter, _ *http.Request) {
			head := 101
			...
			fmt.Fprintf(w, `{"data":{"head_slot":"%d","sync_distance":"0","is_syncing":false,"is_optimistic":false,"el_offline":false}}`, head)
		})
		mux.HandleFunc(peerCountPath, func(w http.ResponseWriter, _ *http.Request) {
			fmt.Fprint(w, `{"data":{"connected":"1"}}`)
		})
		mux.HandleFunc(finalityPath, func(w http.ResponseWriter, _ *http.Request) {
			fmt.Fprint(w, `{"execution_optimistic":false,"data":{"finalized":{"epoch":"0","root":"0xabc"}}}`)
		})
		server := httptest.NewServer(mux)
	...
	check := &systemCheck{
		cfg: config{
			...
			clURLs:                [2]string{cl1.URL, cl2.URL},
		},
		http:                         httpReader{client: &http.Client{Timeout: time.Second}},
		lastValidatorContinuityCheck: time.Now(),
		now: func() time.Time {
			index := int(clockCalls.Add(1) - 1)
			if index >= len(slots) {
				index = len(slots) - 1
			}
			return time.Unix(1700000000+slots[index]*int64(beaconSecondsPerSlot), 0)
		},
	}
	baseline := validatorDutySnapshots{}
	baseline[1].activePubkeys = map[string]struct{}{pubkey: {}}
	duty, err := check.waitValidatorProposalWindow(t.Context(), baseline, validatorProposalProofLeadSlots)
	if err != nil {
		t.Fatal(err)
	}
	if duty.slot != 107 {
		t.Fatalf("selected duty slot %d, want later safe slot 107", duty.slot)
	}
	if calls := clockCalls.Load(); calls < 4 {
		t.Fatalf("clock sampled %d times, want a post-status resample and retry", calls)
	}
}

func TestCanonicalScheduledBeaconHeader(t *testing.T) {
	mux := http.NewServeMux()
	mux.HandleFunc(fmt.Sprintf(beaconHeaderPath, 131), func(w http.ResponseWriter, _ *http.Request) {
		fmt.Fprint(w, `{"execution_optimistic":false,"data":{"root":"0xAABB","canonical":true,"header":{"message":{"slot":"131","proposer_index":"64"}}}}`)
	})
	server := httptest.NewServer(mux)
	defer server.Close()
	header, err := (httpReader{client: server.Client()}).canonicalBeaconHeader(t.Context(), server.URL, 131)
	if err != nil {
		t.Fatal(err)
	}
	if header.root != "0xaabb" || header.slot != 131 || header.proposerIndex != 64 {
		t.Fatalf("unexpected header: %+v", header)
	}
}

func TestWaitScheduledValidatorProposal(t *testing.T) {
	duty := proposerDuty{pubkey: "vc2-selected", validatorIndex: 64, slot: 131}
	newHeaderServer := func(t *testing.T, root string) *httptest.Server {
		t.Helper()
		mux := http.NewServeMux()
		mux.HandleFunc(genesisPath, func(w http.ResponseWriter, _ *http.Request) {
			fmt.Fprint(w, `{"data":{"genesis_time":"1700000000"}}`)
		})
		mux.HandleFunc(fmt.Sprintf(beaconHeaderPath, duty.slot), func(w http.ResponseWriter, _ *http.Request) {
			fmt.Fprintf(w, `{"execution_optimistic":false,"data":{"root":%q,"canonical":true,"header":{"message":{"slot":"131","proposer_index":"64"}}}}`, root)
		})
		server := httptest.NewServer(mux)
		t.Cleanup(server.Close)
		return server
	}
	head1 := newHeaderServer(t, "0xabc")
	head2 := newHeaderServer(t, "0xabc")

	for _, test := range []struct {
		name       string
		afterCalls func(int64) (successful, failed int)
		want       string
	}{
		{name: "canonical success", afterCalls: func(calls int64) (int, int) {
			if calls > 1 {
				return 1, 0
			}
			return 0, 0
		}},
		{name: "proposal failure", afterCalls: func(calls int64) (int, int) {
			if calls > 1 {
				return 0, 1
			}
			return 0, 0
		}, want: "failed proposals by the selected VC2 validator"},
	} {
		t.Run(test.name, func(t *testing.T) {
			var calls atomic.Int64
			metrics := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, _ *http.Request) {
				successful, failed := test.afterCalls(calls.Add(1))
				fmt.Fprintf(w, "# TYPE validator_successful_proposals counter\nvalidator_successful_proposals{pubkey=%q} %d\n", duty.pubkey, successful)
				fmt.Fprintf(w, "# TYPE validator_failed_proposals counter\nvalidator_failed_proposals{pubkey=%q} %d\n", duty.pubkey, failed)
			}))
			defer metrics.Close()
			check := &systemCheck{
				cfg: config{
					...
					pollInterval:          time.Millisecond,
					validatorPollInterval: time.Hour,
					clURLs:                [2]string{head1.URL, head2.URL},
					vcMetricsURLs:         [2]string{"http://unused.invalid", metrics.URL},
				},
				http:                         httpReader{client: &http.Client{Timeout: time.Second}},
				lastValidatorContinuityCheck: time.Now(),
				now:                          func() time.Time { return time.Unix(1700000000+127*int64(beaconSecondsPerSlot), 0) },
			}
			err := check.waitScheduledValidatorProposal(t.Context(), duty, validatorDutySnapshots{})
			if test.want == "" {
				if err != nil {
					t.Fatal(err)
				}
...
func TestScheduledProposalRejectsCounterBaselineThatCrossesWindow(t *testing.T) {
	duty := proposerDuty{pubkey: "vc2-selected", validatorIndex: 64, slot: 131}
	genesis := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
	...
	defer genesis.Close()
	metrics := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, _ *http.Request) {
		fmt.Fprintf(w, "# TYPE validator_successful_proposals counter\nvalidator_successful_proposals{pubkey=%q} 0\n", duty.pubkey)
		fmt.Fprintf(w, "# TYPE validator_failed_proposals counter\nvalidator_failed_proposals{pubkey=%q} 0\n", duty.pubkey)
	}))
	defer metrics.Close()
	var clockCalls atomic.Int64
	check := &systemCheck{
		cfg: config{
			clURLs:        [2]string{"http://unused.invalid", genesis.URL},
			vcMetricsURLs: [2]string{"http://unused.invalid", metrics.URL},
		},
		http: httpReader{client: &http.Client{Timeout: time.Second}},
		now: func() time.Time {
			...
			return time.Unix(1700000000+slot*int64(beaconSecondsPerSlot), 0)
		},
	}
	err := check.waitScheduledValidatorProposal(t.Context(), duty, validatorDutySnapshots{})
	var stale *proposerBaselineWindowError
	if !errors.As(err, &stale) {
		t.Fatalf("got %v, want proposer-baseline-window error", err)
	}
}
