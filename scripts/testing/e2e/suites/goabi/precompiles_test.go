...
func TestMLDSA87VerifyVector(t *testing.T) {
	input, expected, err := loadMLDSA87VerifyVector()
	if err != nil {
		t.Fatal(err)
	}
	if got, want := len(input), mldsa87VerifyContextOffset+int(input[mldsa87VerifyContextLenOffset]); got != want {
		t.Fatalf("ML-DSA-87 input length = %d, want %d", got, want)
	}
	if len(input) == mldsa87VerifyContextOffset {
		t.Fatal("ML-DSA-87 test vector must have a non-empty context")
	}

	precompile := vm.PrecompiledContractsZond[common.BytesToAddress([]byte{3})]
	if precompile == nil {
		t.Fatal("production ML-DSA-87 verify precompile is not registered at 0x03")
	}
	run := func(input []byte) []byte {
		t.Helper()
		output, _, err := vm.RunPrecompiledContract(precompile, input, precompile.RequiredGas(input))
		if err != nil {
			t.Fatalf("run production ML-DSA-87 verify precompile: %v", err)
		}
		return output
	}

	if output := run(input); !bytes.Equal(output, expected) {
		t.Fatalf("valid ML-DSA-87 verification output = %x, want %x", output, expected)
	}
	invalidSignature := bytes.Clone(input)
	invalidSignature[mldsa87VerifySignatureOffset] ^= 0x01
	if output := run(invalidSignature); len(output) != 0 {
		t.Fatalf("invalid ML-DSA-87 signature output = %x, want empty", output)
	}
	invalidContext := bytes.Clone(input)
	invalidContext[mldsa87VerifyContextOffset] ^= 0x01
	if output := run(invalidContext); len(output) != 0 {
		t.Fatalf("invalid ML-DSA-87 context output = %x, want empty", output)
	}
}

func TestMLDSA87VerifyVectorValidation(t *testing.T) {
	validInput, validExpected, err := loadMLDSA87VerifyVector()
	if err != nil {
		t.Fatal(err)
	}
	tests := []struct {
		...
		mutateExpected func([]byte) []byte
	}{
		{
			name: "short fixed frame",
			mutateInput: func(input []byte) []byte {
				return input[:mldsa87VerifyContextOffset-1]
			},
		},
		{
		...
		{
			name: "wrong context length",
			mutateInput: func(input []byte) []byte {
				input[mldsa87VerifyContextLenOffset]--
				return input
			},
		},
		{
			name: "wrong true word",
			mutateExpected: func(expected []byte) []byte {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			input := bytes.Clone(validInput)
			expected := bytes.Clone(validExpected)
			if test.mutateInput != nil {
				input = test.mutateInput(input)
			}
			if test.mutateExpected != nil {
				expected = test.mutateExpected(expected)
			}
			if err := validateMLDSA87VerifyVector(input, expected); err == nil {
				t.Fatal("validateMLDSA87VerifyVector accepted malformed vector")
			}
		})
	}
}
