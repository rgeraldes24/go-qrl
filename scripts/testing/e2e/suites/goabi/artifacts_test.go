...
func TestEventEmitterArtifactsInSync(t *testing.T) {
	...
	if EventEmitterMetaData.ABI != wantBindingABI {
		t.Fatal("generated binding ABI differs from EventEmitter.abi; run go -C scripts/testing/e2e generate ./suites/goabi")
	}
	if normalizeHex(EventEmitterMetaData.Bin) != normalizeHex(binText) {
		t.Fatal("generated binding bytecode differs from EventEmitter.bin; run go -C scripts/testing/e2e generate ./suites/goabi")
	}

	abiLine := "  abi: " + abiText + ",\n"
	binLine := "  bin: \"0x" + binText + "\"\n"
	if strings.Count(jsText, abiLine) != 1 || strings.Count(jsText, binLine) != 1 {
		t.Fatal("emitter.js differs from EventEmitter.abi/EventEmitter.bin; run go run generate_emitter_js.go in testdata/contracts")
	}
}

func TestEventEmitterStorageLayoutUsesVM64Slots(t *testing.T) {
	fixture := filepath.Join("..", "..", "testdata", "contracts", "EventEmitter_storage.json")
