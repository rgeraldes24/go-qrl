...
func checkAddressUpperHalfIsolation(ctx context.Context, run *suiteRun, client *qrlclient.Client, w wallet.Wallet, from common.Address) error {
	...
	var nonce uint64
	if prepared := run.prepared[TransactionAddressIsolationFirst]; prepared != nil {
		// Prefer the durable raw transaction over a hash-only lookup. sendValue
		// validates its label semantics and reconciles these exact bytes before
		// any RPC lookup below can occur.
		nonce = prepared.Nonce()
	} else if recorded, ok := run.transactionForLabel(TransactionAddressIsolationFirst); ok {
		tx, pending, err := client.TransactionByHash(ctx, recorded.hash)
		if err != nil {
			return fmt.Errorf("read recorded upper-half fixture transaction %s: %w", recorded.hash, err)
		}
		if pending {
			return fmt.Errorf("recorded upper-half fixture transaction %s is still pending", recorded.hash)
		}
		nonce = tx.Nonce()
	} else {
		var err error
	...
	firstReceipt, err := sendValue(ctx, run, TransactionAddressIsolationFirst, client, w, from, first, firstValue)
	if err != nil {
	...
	secondReceipt, err := sendValue(ctx, run, TransactionAddressIsolationSecond, client, w, from, second, secondValue)
	if err != nil {
