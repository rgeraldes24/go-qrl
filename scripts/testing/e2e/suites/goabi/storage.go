...
func checkStorageAPIs(ctx context.Context, run *suiteRun, graphqlURL string, client *qrlclient.Client, w wallet.Wallet, from common.Address) error {
	var value [common.StorageValue64Length]byte
	for i := range value {
		value[i] = byte(i + 1)
	}
	receipt, err := deployRaw(ctx, run, TransactionStorageContractDeploy, client, w, from, storageContractCode(value))
	if err != nil {
