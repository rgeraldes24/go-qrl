...
func readContractState(ctx context.Context, client *qrlclient.Client, address common.Address, parsed abi.ABI) (contractState, error) {
	...
	var countOutput []any
	if err := contract.Call(&bind.CallOpts{Context: ctx}, &countOutput, "get_deposit_count"); err != nil {
	...
	var rootOutput []any
	if err := contract.Call(&bind.CallOpts{Context: ctx}, &rootOutput, "get_deposit_root"); err != nil {
...
func submitDeposit(ctx context.Context, client *qrlclient.Client, fundingSeed string, address common.Address, data depositData, parsed abi.ABI) (depositResult, error) {
	...
	if err := client.SendTransaction(ctx, tx); err != nil {
		return depositResult{}, fmt.Errorf("send deposit transaction %s: %w", tx.Hash().Hex(), err)
	}
